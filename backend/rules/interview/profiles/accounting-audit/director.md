---
rule_id: "rule:profile:accounting-audit:director"
kind: "profile"
...
schema_version: 2
---
# Profile phỏng vấn: Kế toán & Kiểm toán / Director

## Phạm vi áp dụng

Blueprint mặc định cho **Kế toán & Kiểm toán** ở level **Director**, ánh xạ
...
3. Mỗi câu phải trả về `grounding_ids` chứa `rule:profile:accounting-audit:director` và ít nhất một evidence ID hợp lệ khác.
...
| 2 | Situational | reconciliation và độ chính xác | Giả sử deadline báo cáo xung đột với chất lượng bằng chứng. Bạn sẽ xử lý theo thứ tự nào, dựa trên tiêu chí nào và khi nào cần escalation? | Kiểm tra sai lệch, materiality và tác động báo cáo. |
...
| 4 | Stakeholder + risk | materiality và risk assessment | Trong tình huống management phản đối audit adjustment, bạn sẽ phối hợp các bên ra sao mà vẫn bảo vệ materiality và risk assessment và chuẩn nghề nghiệp? | Kiểm tra sampling, test procedure và kết luận và điểm dừng rủi ro. |
| 5 | Reflection/level | độc lập và đạo đức | Hãy mô tả một lần kết quả chưa đạt kỳ vọng trong phạm vi kết quả cấp đơn vị hoặc tổ chức trong nhiều quý/năm. Bạn nhận ra điều gì, thay đổi cơ chế nào và bằng chứng nào cho thấy lần sau tốt hơn? | Yêu cầu cách xử lý disagreement với client/management. |
