# -*- coding: utf-8 -*-
"""
Script to update KLTN_InterV_LeMinhDuy_v18.pptx to perfectly reflect the latest InterV codebase
and embed the 15-minute presentation speaking script into slide notes.
"""
...
def update_pptx():
    src_path = r"d:\project\InterV\Review\KLTN_InterV_LeMinhDuy_v18.pptx"
    prs = pptx.Presentation(src_path)
    
    # 1. Update Slide 24 (index 23)
    slide24 = prs.slides[23]
    for shape in slide24.shapes:
        if shape.has_text_frame:
            for p in shape.text_frame.paragraphs:
                for r in p.runs:
                    if r.text.strip() == "54 / 67":
                        r.text = "61 / 74"
                    elif r.text.strip() == "18":
                        r.text = "19"
                    elif r.text.strip() == "14":
                        r.text = "16"
                    elif "39 trang frontend" in r.text:
                        r.text = r.text.replace("39 trang frontend", "38 trang frontend")
                        
    # 2. Update Slide 25 (index 24)
    slide25 = prs.slides[24]
    for shape in slide25.shapes:
        if shape.has_text_frame:
            for p in shape.text_frame.paragraphs:
                for r in p.runs:
                    if r.text.strip() == "38/38":
                        r.text = "51/51"
                    elif r.text.strip() == "14":
                        r.text = "16"
                    elif "4,106 giây" in r.text:
                        r.text = "Đạt 100% trong 3,28 giây"
                    elif "backend 38/38" in r.text:
                        r.text = r.text.replace("backend 38/38", "backend 51/51")

    # 3. Update Slide 23 (index 22) - Tech Stack
    slide23 = prs.slides[22]
    for shape in slide23.shapes:
        if shape.has_text_frame:
            for p in shape.text_frame.paragraphs:
                for r in p.runs:
                    if r.text.strip() == "Python • gRPC":
                        r.text = "Python • gRPC (16 RPCs)"
                    elif "AssemblyAI • Whisper" in r.text:
                        r.text = "AssemblyAI • Faster-Whisper"

    # 4. Update Slide 9 (index 8) - Architecture
    slide9 = prs.slides[8]
    for shape in slide9.shapes:
        if shape.has_text_frame:
            for p in shape.text_frame.paragraphs:
                for r in p.runs:
                    if "Python/gRPC, DeepSeek" in r.text:
                        r.text = "Python/gRPC (16 RPCs), DeepSeek, Qdrant, speech"

    # 5. Populate timed 15-minute speaking notes into each slide
    speaking_notes = {
        1: (
    ...
    # Inject notes into each slide
    for slide_num, note_text in speaking_notes.items():
