# -*- coding: utf-8 -*-
"""
Master script to build the complete 37-slide presentation:
- Adds top-left '#xx' badges (#01 .. #06) on all content slides.
- Creates 6 Section Divider slides matching Slide 2's 6 sections.
- Inserts section divider slides at exact positions backwards.
- Injects speaking notes for all 37 slides.
- Synchronizes all Word scripts and documentation.
"""
...
def build_37_slides():
    ...
    # 1. Delete old slide 18 (index 17: 'Phân rã module theo ranh giới nghiệp vụ')
    rId = prs.slides._sldIdLst[17].rId
    prs.part.drop_rel(rId)
    del prs.slides._sldIdLst[17]
    print(f"Remaining content slides after deleting old Slide 18: {len(prs.slides)}") # 31 slides
    
    # 2. Section mapping on the 31 original content slides (0-indexed)
    sections_def = [
        {
            "sec_num": "01",
            "tag": "#01",
            ...
            "orig_idx": 2,
            "slide_indices": [2, 3, 4, 5] # Slides 3, 4, 5, 6
        },
        {
            "sec_num": "02",
            "tag": "#02",
        ...
        },
        {
            ...
            "orig_idx": 8,
            "slide_indices": [8, 9, 10, 11, 12, 13, 14, 15, 16] # Slides 9..17
        },
        {
            "sec_num": "04",
            "tag": "#04",
            ...
            "orig_idx": 17,
            "slide_indices": [17, 18, 19, 20, 21] # Slides 18..22
        },
        {
            "sec_num": "05",
            "tag": "#05",
            ...
            "orig_idx": 22,
            "slide_indices": [22, 23, 24, 25] # Slides 23..26
        },
        {
            "sec_num": "06",
            "tag": "#06",
            ...
            "orig_idx": 26,
            "slide_indices": [26, 27, 28, 29, 30] # Slides 27..31
        }
    ]
    
    # 3. Add top-left #xx badge to every content slide
    for sec in sections_def:
        ...
        for s_idx in sec["slide_indices"]:
            ...
            # Check if there is an existing badge at top-left
            existing_badge = None
            for shape in slide.shapes:
            ...
            if existing_badge:
                existing_badge.text_frame.clear()
                p = existing_badge.text_frame.paragraphs[0]
                p.text = tag_text
                ...
                # Add top-left badge text box
                tx_box = slide.shapes.add_textbox(left=713225, top=200000, width=1200000, height=260000)
    ...
    # 4. Insert 6 Section Divider slides backwards using original indices
    for sec in reversed(sections_def):
        d_slide = prs.slides.add_slide(prs.slide_layouts[1]) # SECTION_HEADER layout
        
        # Configure Placeholders
        for shape in d_slide.shapes:
            if shape.is_placeholder:
                ph_idx = shape.placeholder_format.idx
                # Number Placeholder (Title 2, idx 2)
                ...
                # Main Title Placeholder (Title 1, idx 0)
                elif ph_idx == 0 or (shape.top >= 2000000 and shape.top < 3000000):
                    shape.text_frame.clear()
                    p = shape.text_frame.paragraphs[0]
                    p.text = sec["title"]
                    p.font.name = "Cambria"
                    ...
                    p.alignment = PP_ALIGN.CENTER
                # Subtitle Placeholder (Subtitle 3, idx 1)
                elif ph_idx == 1 or shape.top >= 3000000:
                    shape.text_frame.clear()
                    p = shape.text_frame.paragraphs[0]
                    p.text = sec["subtitle"]
                    p.font.name = "Montserrat"
        ...
        # In lxml, insert automatically moves the element from the end to orig_idx
    ...
    # 5. Populate speaking notes for all 37 slides
    speaking_notes_37 = {
        1: (
    ...
    prs.save(v20_path)
    print(f"Saved {v20_path} with {len(prs.slides)} slides successfully.")
    
    try:
        prs.save(v18_path)
        print(f"Saved {v18_path} with {len(prs.slides)} slides successfully.")
    except PermissionError:
        pass

def clean_resave():
    for path in [r"d:\project\InterV\Review\KLTN_InterV_LeMinhDuy_v20.pptx", r"d:\project\InterV\Review\KLTN_InterV_LeMinhDuy_v18.pptx"]:
        if os.path.exists(path):
            prs = pptx.Presentation(path)
            prs.save(path)
            print(f"Cleanly resaved {os.path.basename(path)}")

def update_markdown_and_scripts_37():
    v20_path = r"d:\project\InterV\Review\KLTN_InterV_LeMinhDuy_v20.pptx"
    prs = pptx.Presentation(v20_path)
    
    # 1. Update current_slides_analysis.md
    md_path = r"d:\project\InterV\Review\current_slides_analysis.md"
    new_md_lines = [f"# PHÂN TÍCH VÀ NỘI DUNG TỪNG SLIDE ({os.path.basename(v20_path)} - {len(prs.slides)} Slides)\n\n"]
    for idx, slide in enumerate(prs.slides):
...
if __name__ == "__main__":
    build_37_slides()
    clean_resave()
    update_markdown_and_scripts_37()
    print("ALL 37 SLIDES FULLY SYNCHRONIZED AND READY!")
